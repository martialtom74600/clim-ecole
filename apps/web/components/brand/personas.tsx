      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {PERSONA_LIST.map((p) => (
            ...
            >
              <p className="text-xs font-semibold uppercase tracking-wide text-ink-subtle">{p.shortLabel}</p>
              <h3 className="mt-2 font-semibold">{p.headline}</h3>
              <p className="mt-3 text-sm leading-relaxed text-ink-muted">{p.description}</p>
              <span className="mt-4 inline-flex items-center gap-1 text-xs font-medium text-ink group-hover:underline">
                En savoir plus
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-subtle">Finance</p>
