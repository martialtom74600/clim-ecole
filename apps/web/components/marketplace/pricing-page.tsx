...
export function PricingPage({
  ...
  const { label, headline, subline } = PAGE_VERDICTS.tarifs;
  ...
  return (
    <div>
      ...
      <NarrativeSection
        ...
        description="Suffisant pour prioriser vos territoires."
      >
      ...
      <NarrativeSection
        ...
        description="Débloquez quand vous êtes prêt à contacter et chiffrer."
      >
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {PRICING_TIERS.map((plan) => {
            ...
            return (
              <Spotlight
                key={plan.id}
                glow={flagship ? 'light' : 'dark'}
                className={cn(
                  'card flex flex-col p-6 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:shadow-raised',
                  (plan.highlight || highlightPlan === plan.id) &&
                    'border-ink ring-1 ring-ink/10 shadow-raised',
                  flagship && 'bg-ink text-white lg:col-span-1',
                )}
              >
                <div className="flex items-center gap-2">
                  <h3 className={cn('font-semibold', flagship ? 'text-white' : 'text-ink')}>
                    {plan.name}
                  </h3>
                  {flagship && (
                ...
                <p className={cn('mt-1 text-sm', flagship ? 'text-white/70' : 'text-ink-muted')}>
                  {plan.desc}
                </p>
                <p
                  className={cn(
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      flagship ? 'text-white/60' : 'text-ink-muted',
                    )}
                  >
                    {plan.period}
                  </span>
      ...
      <NarrativeSection title="Le parcours" description="Comment utiliser chaque offre.">
        <SiteJourneySteps compact />
      </NarrativeSection>

      <div className="page-content border-t border-line pt-8">
        <p className="text-sm text-ink-muted">
          <GlossaryTerm term="MGPE-PD">MGPE-PD</GlossaryTerm> : contrat de performance énergétique
          permettant à la collectivité de rembourser les travaux via les économies d&apos;énergie.
        </p>
        <p className="mt-4 text-xs text-ink-subtle">{COPY.estimatesNote}</p>
        <Link href="/explorer" className="btn-secondary mt-6 inline-flex">
          {COPY.openExplorer}
        </Link>
      </div>
    </div>
