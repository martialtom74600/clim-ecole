...
function LockedMapPlaceholder() {
  return (
    <div className="flex h-full min-h-[12rem] flex-col items-center justify-center bg-surface-sunken p-6 text-center">
      <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-line bg-white">
        <MapPin className="h-5 w-5 text-ink-subtle" />
      </div>
      <p className="mt-4 text-sm font-medium text-ink-soft">Carte des établissements</p>
...
export function DossierTabFinanceLocked({
  ...
  return (
    <div className="h-full min-h-0 overflow-y-auto bg-surface-sunken">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 p-4 md:p-5 lg:grid-cols-[3fr_7fr]">
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-line bg-white p-4 shadow-card">
            <p className="text-sm font-semibold text-ink">Aperçu financier</p>
            <p className="mt-0.5 text-[11px] text-ink-muted">Tranches indicatives — montants exacts après achat</p>
            <dl className="mt-3 space-y-2.5">
              {previewItems.map(({ label, value, hint }) => (
                <div key={label} className="flex items-start justify-between gap-3 border-b border-line pb-2.5 last:border-0 last:pb-0">
                  <dt className="text-[11px] text-ink-muted">{label}</dt>
                  <dd className="text-right">
                    <span className="block font-mono text-sm font-semibold tabular-nums text-ink">{value}</span>
                    {hint && <span className="mt-0.5 block text-[10px] text-ink-subtle">{hint}</span>}
                  </dd>
                </div>
              ))}
            </dl>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl border border-dashed border-line bg-white p-5">
            ...
            <p className="mt-2 text-xs leading-relaxed text-ink-muted">
              Synthèse décisionnelle, waterfall de financement, simulateur reste à charge et pitch
              MGPE-PD — inclus dans le dossier débloqué.
            </p>
          </div>
...
export function DossierTabProspectLocked({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 bg-surface-sunken p-4 lg:grid lg:grid-cols-12">
      ...
      <div className="hidden min-h-0 overflow-hidden rounded-xl border border-line bg-white lg:col-span-5 lg:flex lg:h-full lg:flex-col">
        <LockedMapPlaceholder />
      </div>
    </div>
  );
          ...
          {['CSV territoire', 'CSV complet', COPY.exportMgpeHtml, 'Note PDF'].map((label) => (
            ...
            >
              ...
              <span>
                ...
                <span className="mt-0.5 block text-xs text-ink-subtle">Après déblocage</span>
              </span>
            </div>
          ))}
        </div>
